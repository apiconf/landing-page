...
export default function ConfirmationMain() {
  ...
  return (
    <div className={`${Styles.FlexContainer} mb-12 xl:mb-40 min-[1440px]:mb-56`}>
...
function ConfirmationHeader() {
  return (
    <div className="text-[#F1F1F1]">
      ...
      <p className="mt-4 text-sm font-medium md:text-lg">Make a shareable image for yourself</p>
    </div>
  );
}
...
function ConfirmationForm({
  ...
  return (
    <form className="flex flex-col gap-y-8" method="post" onSubmit={formSubmitHandler}>
      ...
      <label htmlFor="your-picture" className={Styles.YourPicture}>
        <span className="font-extrabold text-[#2F20BF]">{pictureLabel}</span>
        <ImageIcon />
      </label>
      <input
        type="file"
        name="your-picture"
        id="your-picture"
        className="sr-only h-0 w-0 cursor-pointer opacity-0"
        multiple={false}
        accept="image/*"
        ...
        <div ref={ref} className={Styles.ImagePreview}>
          <div className={Styles.ImagePreviewImageContainer}>
            <img
              src={imageSource}
              alt="You!"
              className="aspect-square w-[35%] rounded-xl bg-white object-cover object-center md:mt-0 md:w-[39.5%] md:rounded-3xl"
            />
