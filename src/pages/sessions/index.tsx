...
type sessions = {
  ...
  job: string;
  employer: string;
  sessionType: 'WORKSHOP' | 'SESSION' | 'PANEL SESSION' | 'SESSION BREAKOUT' | 'LIGHTNING TALK';
  talkTitle: string;
  image: string;
};
...
const cardData: sessions[] = [
  ...
  },
  {
    ...
    talkTitle: 'From Design to Documentation: Building Interactive OpenAPI Specification',
  ...
  },
  {
    ...
    employer: 'FCMB Group',
    sessionType: 'SESSION',
    talkTitle: 'Metrics that Matter: Measuring the Success of Your API Strategy',
  ...
  },
  {
    ...
    employer: '',
    sessionType: 'LIGHTNING TALK',
    talkTitle: 'Navigating the Fog: Effective Product Discovery for API Products',
  ...
  },
  {
    ...
    sessionType: 'LIGHTNING TALK',
    talkTitle: 'Get to know AsyncAPI Initiative',
...
function Main() {
  ...
  return (
    <main className={styles.Main}>
      ...
      >
        <span className="text-black font-bold text-2xl leading-[29.76px]">SEE FULL SCHEDULE</span>
      </motion.a>
    </main>
  );
}
...
function Grids({ no = null }: { no?: number | null }) {
  ...
  return (
    <>
      <h1 className="text-[#F1F1F1] font-sans text-5xl font-bold mb-4">Sessions</h1>
...
function GridCard({ no, id, speaker, job, employer, sessionType, talkTitle, image }: sessions) {
  return (
    <GridContainer id={id! + 1} no={no}>
      <div className="flex justify-center min-[800px]:justify-between min-[800px]:gap-x-10 xl:gap-x-16">
        <div className="w-full flex flex-col lg:flex-row items-center gap-8">
          <div className="w-40 h-[12.5rem] shrink-0 rounded-2xl bg-[#4536DD]">
            <img src={image} alt={speaker} className="w-full h-full object-cover rounded-2xl" />
          </div>
      ...
      <div className="flex flex-col items-center min-[800px]:items-start min-[800px]:ml-10 xl:ml-16">
        <div className="my-4 rounded-[2rem] px-4 py-1 w-max bg-[#E1EF9A]">
          <span className="text-[#0A0A0A] font-sans text-lg font-bold">{sessionType}</span>
        </div>
...
function GridContainer({
  ...
  return (
    <div className={`${styles.GridContainer} ${no && id && id === no ? styles.noborder : null}`}>
      {children}
    </div>
  );
}
