import { sessionAbstracts } from './sessionAbstracts';
import type { ScheduleData } from './types';

export const scheduleData: ScheduleData = {
  title: 'Your Day at a Glance',
  days: [
    {
      dayNumber: 1,
      label: 'Workshop',
      date: 'Fri 24 Jul',
      sessions: [
        {
          id: 'opening-remark-welcome-networking-fri',
          title: 'Opening remark, welcome, Networking',
          timeSlot: '09:00am - 10:00am',
          room: 'All Halls',
          duration: '1hr',
          timeFrame: '9am',
          type: 'Welcome',
          color: '#D9D9D9',
        },
        {
          id: 'monitoring-apis-with-opentelemetry-fri',
          title: 'Monitoring APIs with OpenTelemetry',
          timeSlot: '10:00am - 10:45am',
          room: 'Mulberry Hall',
          duration: '45m',
          timeFrame: '10am',
          type: 'workshop',
          speaker: 'Ayooluwa Isaiah',
          color: '#F0E6FF',
        },
        {
          id: 'the-lazy-way-to-keep-your-api-docs-in-sync-fri',
          title: 'The lazy way to keep your API Docs in sync',
          timeSlot: '10:00am - 10:45am',
          room: 'Mauve Hall',
          duration: '45m',
          timeFrame: '10am',
          type: 'workshop',
          speaker: 'Prince Onyeanuna',
          color: '#F0E6FF',
        },
        {
          id: 'type-safe-apis-with-hono-and-zod-fri',
          title: 'Type-Safe APIs with Hono and Zod: Fast, Lightweight, and Bulletproof',
          timeSlot: '10:00am - 10:45am',
          room: 'Plum Hall',
          duration: '45m',
          timeFrame: '10am',
          type: 'workshop',
          speaker: 'Chinedu Otutu',
          color: '#F0E6FF',
        },
        {
          id: 'break-1045-fri',
          title: 'Break',
          timeSlot: '10:45am - 11:00am',
          room: 'All Halls',
          duration: '15m',
          timeFrame: '10am',
          type: 'Break',
          color: '#D9D9D9',
        },
        {
          id: 'hacking-apis-in-the-wild-fri',
          title:
            'Hacking APIs in the Wild: Finding, Exploiting & Reporting Modern API Vulnerabilities',
          timeSlot: '11:00am - 11:45am',
          room: 'Mulberry Hall',
          duration: '45m',
          timeFrame: '11am',
          type: 'workshop',
          speaker: 'Tes Sal',
          color: '#F0E6FF',
        },
        {
          id: 'building-your-first-ai-agent-with-genkit-fri',
          title: 'Building Your First AI Agent with Genkit',
          timeSlot: '11:00am - 11:45am',
          room: 'Mauve Hall',
          duration: '45m',
          timeFrame: '11am',
          type: 'workshop',
          speaker: 'Jamiu Okanlawon',
          color: '#F0E6FF',
        },
        {
          id: 'your-api-docs-are-breaking-ai-fri',
          title: 'Your API Docs Are Breaking AI',
          timeSlot: '11:00am - 11:45am',
          room: 'Plum Hall',
          duration: '45m',
          timeFrame: '11am',
          type: 'workshop',
          speaker: 'Funke Olasupo',
          color: '#F0E6FF',
        },
        {
          id: 'break-1145-fri',
          title: 'Break',
          timeSlot: '11:45am - 12:00pm',
          room: 'All Halls',
          duration: '15m',
          timeFrame: '11am',
          type: 'Break',
          color: '#D9D9D9',
        },
        {
          id: 'teaching-ai-coding-assistants-mcp-fri',
          title: 'Teaching AI Coding Assistants to Talk to Real APIs via MCP',
          timeSlot: '12:00pm - 12:45pm',
          room: 'Mulberry Hall',
          duration: '45m',
          timeFrame: '12pm',
          type: 'workshop',
          speaker: 'Muhammad Samu',
          color: '#F0E6FF',
        },
        {
          id: 'break-1245-fri',
          title: 'Break',
          timeSlot: '12:45pm - 01:00pm',
          room: 'All Halls',
          duration: '15m',
          timeFrame: '12pm',
          type: 'Break',
          color: '#D9D9D9',
        },
        {
          id: 'lunch-fri',
          title: 'Lunch',
          timeSlot: '01:00pm - 02:00pm',
          room: 'All Halls',
          duration: '1hr',
          timeFrame: '1pm',
          type: 'session',
          color: '#D9D9D9',
        },
        {
          id: 'stop-re-prompting-reusable-ai-skills-fri',
          title: 'Stop Re-Prompting: How Reusable AI Skills Make Vibe Coding Actually Work',
          timeSlot: '02:00pm - 02:45pm',
          room: 'Mulberry Hall',
          duration: '45m',
          timeFrame: '2pm',
          type: 'workshop',
          speaker: 'Oluwadamilola Oshungboye',
          color: '#F0E6FF',
        },
        {
          id: 'building-secure-scalable-api-infrastructure-azure-fri',
          title:
            'Building Robust, Secure, and Scalable API Infrastructure with Microsoft Azure Services',
          timeSlot: '02:00pm - 02:45pm',
          room: 'Mauve Hall',
          duration: '45m',
          timeFrame: '2pm',
          type: 'workshop',
          speaker: 'Timothy Ogunwemimo',
          color: '#F0E6FF',
        },
        {
          id: 'rethinking-caching-agentic-systems-fri',
          title:
            'Rethinking Caching: Building APIs For The Unpredictable Traffic from Agentic Systems',
          timeSlot: '02:00pm - 02:45pm',
          room: 'Plum Hall',
          duration: '45m',
          timeFrame: '2pm',
          type: 'workshop',
          speaker: 'Alayesanmi Femi',
          color: '#F0E6FF',
        },
        {
          id: 'break-1445-fri',
          title: 'Break',
          timeSlot: '02:45pm - 03:00pm',
          room: 'All Halls',
          duration: '15m',
          timeFrame: '2pm',
          type: 'Break',
          color: '#D9D9D9',
        },
        {
          id: 'from-rest-to-mcp-fastmcp-fri',
          title: 'From REST to MCP: Making Your Backend AI-Ready with FastMCP',
          timeSlot: '03:00pm - 03:45pm',
          room: 'Mulberry Hall',
          duration: '45m',
          timeFrame: '3pm',
          type: 'workshop',
          speaker: 'Abdulqudus Abubakre',
          color: '#F0E6FF',
        },
        {
          id: 'chat-charge-trace-agentic-checkout-fri',
          title: 'Chat, Charge, Trace: Instrumenting an Agentic Checkout Live',
          timeSlot: '03:00pm - 03:45pm',
          room: 'Mauve Hall',
          duration: '45m',
          timeFrame: '3pm',
          type: 'workshop',
          speaker: 'Claret Ibeawuchi & Chukwuemeka Chukwurah',
          sessionSpeakers: [
            {
              name: 'Claret Ibeawuchi',
              role: 'Head of Engineering · Dockie',
              image: '/claret.jpg',
            },
            {
              name: 'Chukwuemeka Chukwurah',
              role: 'Senior Software Engineer · Rocksteady Technologies',
              image: 'https://cdn.sessionize.com/image/564f-400o400o2-JYMeYKCoeZ7rMDrRwyT4HB.jpg',
            },
          ],
          color: '#F0E6FF',
        },
        {
          id: 'from-monolith-to-microservices-fri',
          title:
            'From Monolith to Microservices: Designing Resilient, Observable APIs with Node.js',
          timeSlot: '03:00pm - 03:45pm',
          room: 'Plum Hall',
          duration: '45m',
          timeFrame: '3pm',
          type: 'workshop',
          speaker: 'Usman Soliu',
          color: '#F0E6FF',
        },
        {
          id: 'closing-photos-fri',
          title: 'Closing, Photos',
          timeSlot: '03:45pm - 04:00pm',
          room: 'All Halls',
          duration: '15m',
          timeFrame: '3pm',
          type: 'session',
          color: '#D9D9D9',
        },
      ],
    },
    {
      dayNumber: 2,
      label: 'Conference',
      date: 'Sat 25 Jul',
      sessions: [
        {
          id: 'welcome-and-networking-sat',
          title: 'Welcome and Networking',
          timeSlot: '09:00am - 09:50am',
          room: 'All Halls',
          duration: '50m',
          timeFrame: '9am',
          type: 'Welcome',
          color: '#D9D9D9',
        },
        {
          id: 'opening-address-sat',
          title: 'Opening Address',
          timeSlot: '09:50am - 10:00am',
          room: 'All Halls',
          duration: '10m',
          timeFrame: '9am',
          type: 'Welcome',
          color: '#D9D9D9',
        },
        {
          id: 'from-prompts-to-autonomous-systems-sat',
          title:
            'From Prompts to Autonomous Systems: Architecting Production-Ready AI Agents on Google Cloud',
          timeSlot: '10:00am - 10:20am',
          room: 'Ruby Hall',
          duration: '20m',
          timeFrame: '10am',
          type: 'Lightning',
          speaker: 'Peterson Nwoko',
          color: '#90EAF2',
        },
        {
          id: 'announcement-1020-sat',
          title: 'Announcement',
          timeSlot: '10:20am - 10:30am',
          room: 'All Halls',
          duration: '10m',
          timeFrame: '10am',
          type: 'Announcement',
          color: '#D9D9D9',
        },
        {
          id: 'where-will-ai-converge-sat',
          title: 'Where will AI Converge: No intelligence without Energy',
          timeSlot: '10:45am - 11:00am',
          room: 'Ruby Hall',
          duration: '15m',
          timeFrame: '10am',
          type: 'Lightning',
          speaker: 'Ibukun Adedeji',
          color: '#90EAF2',
        },
        {
          id: 'the-assumptions-that-break-apis-in-production-sat',
          title: 'The Assumptions That Break APIs in Production',
          timeSlot: '10:30am - 10:45am',
          room: 'Ruby Hall',
          duration: '15m',
          timeFrame: '10am',
          type: 'Lightning',
          speaker: 'Ayomide Ariyo',
          color: '#90EAF2',
        },
        {
          id: 'the-network-knows-sat',
          title:
            "The Network Knows: Leveraging Telco Capabilities to Solve Fintech's Hardest Problems",
          timeSlot: '11:00am - 11:15am',
          room: 'Ruby Hall',
          duration: '15m',
          timeFrame: '11am',
          type: 'Keynote',
          speaker: 'Chilezie Unachukwu',
          color: '#ECC89D',
        },
        {
          id: 'ice-breaker-sat',
          title: 'Ice breaker',
          timeSlot: '11:15am - 11:25am',
          room: 'All Halls',
          duration: '10m',
          timeFrame: '11am',
          type: 'session',
          color: '#D9D9D9',
        },
        {
          id: 'ai-agents-need-guardrails-sat',
          title: 'AI Agents Need Guardrails: Securing APIs in the Age of Autonomous Systems',
          timeSlot: '11:25am - 11:55am',
          room: 'Ruby Hall',
          duration: '30m',
          timeFrame: '11am',
          type: 'session',
          speaker: 'Obi Uchenna David',
          color: '#E1EF8B',
        },
        {
          id: 'governance-without-the-bureaucracy-sat',
          title:
            'Governance Without the Bureaucracy: How to Enforce API Standards Without Slowing Down Your Team',
          timeSlot: '11:25am - 11:55am',
          room: 'Emerald Hall',
          duration: '30m',
          timeFrame: '11am',
          type: 'session',
          speaker: 'Amarachi Iheanacho',
          color: '#E1EF8B',
        },
        {
          id: 'software-is-going-headless-sat',
          title: 'Software Is Going Headless: What the API Becomes When Agents Run Everything',
          timeSlot: '11:25am - 11:55am',
          room: 'Sapphire Hall',
          duration: '30m',
          timeFrame: '11am',
          type: 'session',
          speaker: 'Joel Olawanle',
          color: '#E1EF8B',
        },
        {
          id: 'break-1155-sat',
          title: 'Break',
          timeSlot: '11:55am - 12:05pm',
          room: 'All Halls',
          duration: '10m',
          timeFrame: '11am',
          type: 'Break',
          color: '#D9D9D9',
        },
        {
          id: 'designing-apis-as-products-sat',
          title: 'Designing APIs as Products: How Product Thinking Improves Developer Experience',
          timeSlot: '12:05pm - 12:35pm',
          room: 'Ruby Hall',
          duration: '30m',
          timeFrame: '12pm',
          type: 'session',
          speaker: 'Cindy Shontan',
          color: '#E1EF8B',
        },
        {
          id: 'ai-can-write-code-sat',
          title: 'AI Can Write Code. But It Doesn’t Know Your System',
          timeSlot: '12:05pm - 12:35pm',
          room: 'Emerald Hall',
          duration: '30m',
          timeFrame: '12pm',
          type: 'session',
          speaker: 'Chigozie Madubuko',
          color: '#E1EF8B',
        },
        {
          id: 'building-crypto-payment-apis-in-africa-sat',
          title:
            'Building Crypto Payment APIs in Africa: Stablecoins, On-Ramps & the Regulatory Tightrope',
          timeSlot: '12:05pm - 12:35pm',
          room: 'Sapphire Hall',
          duration: '30m',
          timeFrame: '12pm',
          type: 'session',
          speaker: 'Progress Ochuko Eyaadah (Koxy)',
          color: '#E1EF8B',
        },
        {
          id: 'building-compliant-fintech-apis-sat',
          title: 'Building Compliant Fintech APIs for African Corridors',
          timeSlot: '12:35pm - 01:05pm',
          room: 'Ruby Hall',
          duration: '30m',
          timeFrame: '12pm',
          type: 'session',
          speaker: 'Oluwatoni Akinmolayan',
          color: '#E1EF8B',
        },
        {
          id: 'the-asyncapi-epic-guideline-sat',
          title: 'The AsyncAPI EPIC Guideline',
          timeSlot: '12:35pm - 01:05pm',
          room: 'Emerald Hall',
          duration: '30m',
          timeFrame: '12pm',
          type: 'session',
          speaker: 'Azeez Elegbede',
          color: '#E1EF8B',
        },
        {
          id: 'when-payments-fail-sat',
          title: 'When Payments Fail: Designing Resilient APIs for Fintech Systems',
          timeSlot: '12:35pm - 01:05pm',
          room: 'Sapphire Hall',
          duration: '30m',
          timeFrame: '12pm',
          type: 'session',
          speaker: 'Motunrayo Koyejo',
          color: '#E1EF8B',
        },
        {
          id: 'lunch-sat',
          title: 'Lunch',
          timeSlot: '01:05pm - 02:00pm',
          room: 'All Halls',
          duration: '55m',
          timeFrame: '1pm',
          type: 'session',
          color: '#D9D9D9',
        },
        {
          id: 'panel-session-sat',
          title: 'General Panel Session',
          timeSlot: '02:00pm - 02:30pm',
          room: 'Ruby Hall',
          duration: '30m',
          timeFrame: '2pm',
          type: 'Panel session',
          color: '#D8CCFE',
          panelSpeakers: [
            {
              name: 'Tunji David Obidokun',
              role: 'ML/AI Engineer, ALAT by Wema Bank',
              image: '/panelists/tunji-david-obidokun.png',
              bio: 'Tunji David Obidokun is an AI/ML Engineer at ALAT by Wema Bank, where he builds and deploys production-grade AI systems that solve real-world enterprise challenges. His work spans fraud detection, computer vision for facial and identity verification, intelligent document processing, conversational AI, and large language model applications, with a strong focus on scalable, secure, and reliable production systems. His expertise covers the end-to-end AI lifecycle, from data engineering and model development to MLOps, cloud deployment, monitoring, and continuous model improvement. Beyond financial services, Tunji has developed AI-powered learning solutions that adapt to the unique learning patterns of neurodivergent children. He enjoys sharing practical lessons from deploying AI in enterprise environments, including scaling machine learning systems, operationalizing LLMs, and building solutions that deliver measurable business value.',
            },
            {
              name: 'Femi Taiwo',
              role: 'CTO / Google Developer Expert, Cloud · INITS',
              image: '/Femi-Taiwo.png',
              bio: 'Femi Taiwo is a technology executive, Google Developer Expert (Cloud), and Co-Founder/CTO of INITS Limited, with over 24 years’ experience building scalable, impact-driven solutions across Africa. He has led transformative projects in fintech, healthcare, logistics, taxation, identity management, and Digital Public Infrastructure (DPI), including consulting for the World Bank on secure, interoperable systems that advance financial inclusion and public service delivery. A passionate community leader, Femi co-leads Google Developer Groups and DevFest Lagos, one of Africa’s largest tech communities, reaching thousands of developers and entrepreneurs each year. He actively mentors high-growth startups through the Google for Startups Accelerator, running workshops and advising founders on scaling teams, products, and engineering processes. He is deeply committed to applying AI and emerging technologies to solve real-world problems, improve efficiency, and create economic opportunities across the region.',
            },
            {
              name: 'Kanwulia Okafor',
              role: 'Director, Industry Services (Africa), GSMA',
              image: '/panelists/kanwulia-okafor.png',
              bio: 'Kanwulia Okafor is Director of Industry Services for Africa at the GSMA, where she leads initiatives at the intersection of mobile technology, artificial intelligence, and digital transformation across the continent. Her work includes advancing GSMA Open Gateway, supporting the development and deployment of African-language AI, and building partnerships across mobile operators, governments, technology companies, and the wider digital ecosystem. Kanwulia is particularly focused on how trusted mobile network capabilities and standardised APIs can support scalable AI applications, including secure user onboarding, identity verification, authentication, and fraud prevention. She works with stakeholders across Africa to translate emerging technologies into practical use cases, partnerships, and market-led pilots that address locally relevant challenges.',
            },
            {
              name: 'Joshua Chijioke',
              role: 'Head, Chenosis West Africa',
              image: '/panelists/joshua-chijioke.png',
              imagePosition: 'top',
              bio: 'Joshua Chijioke leads Chenosis across West Africa, driving market expansion through innovative products, go-to-market strategy, revenue growth, and strategic partnerships. His entrepreneurial experience includes founding a profitable last-mile delivery startup and ventures focused on insurance and AI-powered rewards and loyalty programmes. Before Chenosis, he held roles at MTN where he drove revenue growth, diversification, and new technology solutions. He holds a BSc in Economics from the University of Ilorin and a Strategy Certification from Harvard Business School, alongside certifications spanning digital marketing, cybersecurity, AI, and product management.',
            },
          ],
        },
        {
          id: 'hackathon-announcement-sat',
          title: 'Hackathon announcement & Demo from winners',
          timeSlot: '02:45pm - 03:15pm',
          room: 'Ruby Hall',
          duration: '30m',
          timeFrame: '2pm',
          type: 'Announcement',
          color: '#D9D9D9',
        },
        {
          id: 'main-stage-3-sat',
          title: 'Are Your APIs Ready for Agents',
          timeSlot: '02:30pm - 02:45pm',
          room: 'Ruby Hall',
          duration: '15m',
          timeFrame: '2pm',
          type: 'Keynote',
          speaker: 'Auwal MS',
          color: '#ECC89D',
        },
        {
          id: 'break-1515-sat',
          title: 'Break',
          timeSlot: '03:15pm - 03:25pm',
          room: 'All Halls',
          duration: '10m',
          timeFrame: '3pm',
          type: 'Break',
          color: '#D9D9D9',
        },
        {
          id: 'what-api-designers-can-learn-from-kubernetes-sat',
          title: 'What API Designers Can Learn from the Kubernetes API',
          timeSlot: '03:25pm - 03:55pm',
          room: 'Ruby Hall',
          duration: '30m',
          timeFrame: '3pm',
          type: 'session',
          speaker: 'Somtochi Onyekwere',
          color: '#E1EF8B',
        },
        {
          id: 'improving-api-adoption-through-dx-sat',
          title: 'Improving API Adoption Through Developer Experience, Not Just Design',
          timeSlot: '03:25pm - 03:55pm',
          room: 'Emerald Hall',
          duration: '30m',
          timeFrame: '3pm',
          type: 'session',
          speaker: 'Onyedikachi Hope Amaechi-Okorie',
          color: '#E1EF8B',
        },
        {
          id: 'when-your-api-is-slow-its-probably-sql-sat',
          title: 'When Your API Is Slow, It’s Probably SQL: Performance Tuning for Production APIs',
          timeSlot: '03:25pm - 03:55pm',
          room: 'Sapphire Hall',
          duration: '30m',
          timeFrame: '3pm',
          type: 'session',
          speaker: 'Jamiu Adam',
          color: '#E1EF8B',
        },
        {
          id: 'break-1555-sat',
          title: 'Break',
          timeSlot: '03:55pm - 04:05pm',
          room: 'All Halls',
          duration: '10m',
          timeFrame: '3pm',
          type: 'Break',
          color: '#D9D9D9',
        },
        {
          id: 'the-apis-you-dont-control-sat',
          title:
            'The APIs You Don’t Control: Building on NIBSS, CBN & Payment Rails Without Losing Your Mind',
          timeSlot: '04:05pm - 04:35pm',
          room: 'Ruby Hall',
          duration: '30m',
          timeFrame: '4pm',
          type: 'session',
          speaker: 'Ifeanyi Echereobia',
          color: '#E1EF8B',
        },
        {
          id: 'designing-better-developer-experience-for-web3-apis-sat',
          title: 'Designing Better Developer Experience for Web3 APIs',
          timeSlot: '04:05pm - 04:35pm',
          room: 'Emerald Hall',
          duration: '30m',
          timeFrame: '4pm',
          type: 'session',
          speaker: 'Nasihudeen Jimoh',
          color: '#E1EF8B',
        },
        {
          id: 'from-scattered-docs-to-instant-answers-langchain-sat',
          title:
            'From Scattered Docs to Instant Answers: How We Built an Internal Knowledge Graph with LangChain',
          timeSlot: '04:05pm - 04:35pm',
          room: 'Sapphire Hall',
          duration: '30m',
          timeFrame: '4pm',
          type: 'session',
          speaker: 'Kelechi Oliver Azorji',
          color: '#E1EF8B',
        },
        {
          id: 'closing-sat',
          title: 'Closing Remarks, Networking, Photos',
          timeSlot: '04:35pm - 05:00pm',
          room: 'All Halls',
          duration: '25m',
          timeFrame: '4pm',
          type: 'session',
          color: '#D9D9D9',
        },
      ],
    },
  ],
};

for (const day of scheduleData.days) {
  for (const session of day.sessions) {
    const abstract = sessionAbstracts[session.id];
    if (abstract) session.abstract = abstract;
  }
}
